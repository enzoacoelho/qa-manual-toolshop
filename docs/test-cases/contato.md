...
* **Passos:**
  1. Preencher todos os campos obrigatórios (Nome, Sobrenome, E-mail, Assunto e Mensagem válidos).
  2. Clicar no botão **Send**.
* **Resultado Esperado:**
  * Os campos são preenchidos corretamente.
  * Mensagem enviada com sucesso e toast/alerta de confirmação exibido na tela (ex: *"Thanks for your message..."*).
...
* **Pré-condições:**
  * Estar na página de Contato (`/contact`).
* **Passos:**
  1. Deixar os campos Assunto e Mensagem em branco (Assunto: `[Selecione]` / Mensagem: `[Vazio]`).
  2. Clicar no botão **Send**.
* **Resultado Esperado:**
  * O formulário não é enviado.
  * Envio bloqueado e mensagens de erro de campos obrigatórios exibidas na tela.
...
* **Pré-condições:**
  * Estar na página de Contato (`/contact`).
* **Passos:**
  1. Tentar anexar um arquivo que não seja do formato `.txt` (Anexo: `documento.pdf` ou `imagem.jpg`).
* **Resultado Esperado:**
  * O sistema exibe mensagem de erro (ex: *"File format not supported"*) e não aceita o anexo.
...
* **Pré-condições:**
  * Estar na página de Contato (`/contact`).
* **Passos:**
  1. Anexar um arquivo `.txt` contendo texto (tamanho > 0 KB, ex: `arquivo_com_texto.txt` com 1 KB).
* **Resultado Esperado:**
  * O sistema indica que o tamanho do arquivo excede o limite permitido (0 KB) e impede o envio.
